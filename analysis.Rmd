...
That looks like a pretty good fit, right? But how do we evaluate how good of a fit the linear regression provides? One function we can use in R is `summary()`. This function provides an overview of the fitted model.  
...
You probably found 0.759 on the table. This means that 75.9% of the variation in flipper length can be explained by the body mass of penguins. That is a pretty high $R^2$, especially for observational data. Therefore, we can already consider this model to be quite good for modeling flipper length. 
...
#   filter(district == "Massachusetts")
# 
lm2 <- lm(sentence_length ~ age + sex + educ + year + mandatory_min + 
            gov_departures + race,
          data = us_sent_mass,
          model = TRUE, 
          y = TRUE)
