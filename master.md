...
- [4 Data exploration](#4-data-exploration)
  ...
  - [4.2 Describe the structure of the
    data](#42-describe-the-structure-of-the-data)
    - [4.2.1 Who](#who)
    - [4.2.2 What](#422-what)
    - [4.2.3 When](#423-when)
    - [4.2.4 Where](#424-where)
...
the severity of the crime (`all_adjustments`). The two are combined,
using a table, to determine the recommended range of the sentence.
...
history category,” represented by the criminal_history values 1 through
6. As an additional example, a defendant has three prior convictions
with sentences of over 13 months and two with sentences between 2 and 13
...
`read_csv()`. To view the data as a spreadsheet in a new tab within
RStudio, you can use the `View()` function. Because this is an
interactive RStudio function that doesn’t work when we knit the
document, we have set up our document so that the code can be viewed
below but not actually run when we knit (specifically, we used the code
chunk option `eval = FALSE`).
...
Let’s inspect our variables which are in the columns. There are several
different functions we can use to do this:
...
Using this final output from `glimpse()` we can try to identify which
variables are of interest. Remember, we want to know *who* is in our
dataset, *what* the sentence was, *when* the individual was sentenced
and *where* the sentence occurred. Can you identify some of the
variables that might correspond with each of these questions? We’ll
define and explore variables in more detail next.
...
![](master_files/figure-gfm/unnamed-chunk-20-1.png)<!-- --> What is the
