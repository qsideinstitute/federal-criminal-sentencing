...
Apart from `criminal_history` and `all_adjustments`, there are several variables related to identity: 
* `age`, which refers to a defendant's age in years.
* `sex`, which has been coded as a binary variable 0 and 1, where 0 is "Male" and 1 is "Female". `educ` refers to a defendant's educational attainment which consists of 6 levels FINISH EXPLAINING HERE.
* `educ`, and 
* `race`. refers to a defendant's race ADD HERE.
...
The nine other variables in the dataset refer to information about the court cases themselves: 
* `year`, 
* `sentence_length`, 
* `guilty_plea`, 
* `base_chapter2_adjustments`, 
* `base_chapter2_3_adjustments`, 
* `grid_cell`, 
* `mandatory_min`, 
* `gov_departures`, and 
* `district`. 
